...
export async function syncToSupabase(userId: string, oldData: AppData, newData: AppData): Promise<void> {
  const promises: PromiseLike<{ table: string; op: string; error: any } | any>[] = [];
  const track = (table: string, op: string, p: PromiseLike<any>) =>
    promises.push(Promise.resolve(p).then((r: any) => ({ table, op, error: r?.error })));
  ...
  if (addedVend.length > 0) {
    track("vendedores", "insert", supabase.from("vendedores").insert(addedVend.map((nome) => ({ user_id: userId, nome }))));
  }
  ...
  const errors = (results as any[]).filter((r) => r?.error);
  if (errors.length > 0) {
    for (const e of errors) {
      console.error(`[syncToSupabase] ${e.table}.${e.op} falhou:`, e.error);
    }
    const first = errors[0];
    const msg = first.error?.message || first.error?.details || "erro desconhecido";
    throw new Error(`${first.table} (${first.op}): ${msg}`);
  }
}
